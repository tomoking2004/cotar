# --- BEGIN local patch: how to read this file -------------------------------
# The official GQA evaluator, upstream plus local patches. Every deviation from
# the original is fenced by `--- BEGIN local patch: ... ---` / `--- END local
# patch ---`, and none of them touch the scoring: what they change is when the
# script refuses to run, and whether an optional metric is computed at all.
#
# The original is `datasets/gqa/eval/eval.py`. It ships in GQA's eval.zip
# alongside val_choices.json, so it is present on any machine that can run
# cotar -- which is why no copy is kept in this tree. (One used to live under
# `ref/`, and a patch leaked into the copy itself, silently voiding the very
# baseline it existed to provide.) The whole of the difference is one command:
#
#     git diff --no-index datasets/gqa/eval/eval.py cotar/data/_gqa_eval.py
#
# The invariant to hold: every change that diff reports sits inside a patch
# fence. Upstream's Python 2 leftovers (`apply()`, the undefined
# `get_slice_obj`) and its dead code are not ours to fix, so this file is kept
# out of the linter's reach (`extend-exclude` in pyproject.toml).
# --- END local patch --------------------------------------------------------
...
# --- BEGIN local patch: optional choices (not in the official GQA evaluator) ---
# Upstream always loads the choices file, but it feeds only validity and
# plausibility -- and GQA ships choices for the train/val tiers alone. Making it
# opt-in (as --consistency and --grounding already are) is what lets testdev be
# scored on every *other* official metric: accuracy, binary/open, distribution
# and the per-type breakdowns. Nothing else in the script reads `choices`.
parser.add_argument('--choices',        default=None,                        type = str,    help = "Choices file name format. Omit to skip validity/plausibility (no choices file ships for testdev).")
# --- END local patch --------------------------------------------------------
...
# Load scene graphs
# --- BEGIN local patch: optional scene graphs (not in the official GQA evaluator) ---
# Upstream loads the scene graphs unconditionally, but only the grounding score
# reads them -- and grounding is already opt-in (--grounding), needs attention maps
# we do not produce, and the sceneGraphs files ship separately and are not part of
# our download. Loading them eagerly makes every run die in loadFile.
scenes = None
if args.grounding:
...
# --- BEGIN local patch: subset support (not in the official GQA evaluator) ---
# The upstream script requires `predictions` to cover every balanced question
# (and *every* question with --consistency) and aborts otherwise. We routinely
# score only a subset of the questions file, so instead we evaluate exactly the
# questions we have a prediction for.
#
# We drop the unscored questions here rather than `continue`-ing past them inside
# the main loop, because filtering up front (a) keeps the official scoring loop
# byte-for-byte unchanged, (b) lets its progress bar reflect the real subset size
# instead of the whole file, and (c) makes every predictions[qid] / choices[qid]
# access below safe by construction. The evaluated/skipped counts are always
# reported on a single line that the wrapper parses back into its result dict.
# (--consistency additionally needs the entailed questions' predictions;
# updateConsistency skips any that are absent -- see its patch.)
scored = {qid: question for qid, question in questions.items() if qid in predictions}
skipped = len(questions) - len(scored)
if not scored:
    raise Exception(
        "none of the {} predictions match the {} questions".format(len(predictions), len(questions))
    )
print("Evaluating {} of {} questions ({} skipped: no prediction).".format(
    len(scored), len(questions), skipped))
questions = scored
# --- END local patch --------------------------------------------------------
...
def updateConsistency(questionId, question, questions):
    # --- BEGIN local patch: subset support (not in the official GQA evaluator) ---
    # An entailed question may fall outside the evaluated subset, so only score
    # those we have both a question entry and a prediction for. On the full
    # dataset every entailed id is present, leaving the result unchanged.
    inferredQuestions = [
        eid for eid in question["entailed"]
...
def chiSquare(goldDist, predictedDist):
    ...
    # --- BEGIN local patch: subset support (not in the official GQA evaluator) ---
    # The distribution metric needs at least one balanced question carrying a
    # global group; a subset may contain none, leaving sumOverall == 0. Mirror
...
for m in metrics:
    ...
    if m == "consistency" and not args.consistency:
        continue
    # --- BEGIN local patch: optional choices (not in the official GQA evaluator) ---
    if m in ("validity", "plausibility") and not args.choices:
        continue
    # --- END local patch --------------------------------------------------------
